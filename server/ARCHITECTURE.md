...
| `Envelope` (`bus/bus.go`) | a marshalled frame plus the two routing fields (`Type`, `From`) fan-out reads without re-parsing | published to `proj:<id>` or `events` on `inProc` or `redisBus`; dropped, not queued, for a slow subscriber | delivered to `session.busCh` and to `serveEvents` subscribers |
