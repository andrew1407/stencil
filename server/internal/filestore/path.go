...
// safeJoin resolves <root>/projects/<id>/<kind>.<ext>, validating every segment and re-checking that the
// cleaned absolute path is still inside the store root — the single choke point against traversal.
...
// confine cleans p (relative to root or absolute) and verifies the result lies within the store root,
// defeating "..", absolute paths and stray separators. Symlink escape is checked at write/read time.
