...
TEST_CASE("rectZoom fills + centers the swept rect (capped at 32x)") {
  // Rect 100x100 at (50,50); viewport 400x400 -> scale min(4,4,32)=4.
  ...
  // Tiny rect (would be 40x) is capped at the 32x ceiling.
  const auto capped = rectZoom(0, 0, 10, 10, 400, 400);
  CHECK(capped.scale == doctest::Approx(32.0));
}
