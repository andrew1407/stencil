...
|---|---|---|
| `browser/js/config/llm/opRegistry.json` (+ [`opRegistry.README.md`](../browser/js/config/llm/opRegistry.README.md)) | the op set: every op's keys/validation, per-profile membership, §1 limits, token regexes, §13 prompt bullets, and every measured cross-surface divergence | `browser/tests/opRegistryCanon.test.js` — pins it to the live `js/llm/plan.js` structures and cross-checks the fixture corpus; every surface's fixture walker proves its table-driven validator against the corpus |
...
and runs a port of the reference engine `browser/js/llm/plan/schema.js`, keeping only its normalizers,
...
`tests/fixture_overrides.json`, `desktop`/`browser-extension` as `tests/fixtureOverrides.json`, `bot` as
