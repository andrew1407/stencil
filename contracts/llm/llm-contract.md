...
| `browser-extension/` | `src/llm/` (each module with a `.d.ts` sibling) — `opSchema.js` and `llmClient.js` are whole-file ports of the browser modules, pinned identical (header and import paths aside) by `tests/portParity.test.js`; `opPlan.js` is the extension's OWN §8 profile module over its `src/config/opRegistry.json` copy | `npm test` llm suites: `llmOpPlan`, `llmClient`, `llmChatController`, `llmSettings`, `dataParity.test.js`, `fixtureWalkers.test.js`, `portParity.test.js` |
