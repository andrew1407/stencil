...
total and per-profile counts are pinned by `browser/tests/llm/plan/opRegistryCanon.test.js`). **Every surface's
...
- **Prompt prose lives in the asset, not in client source.** Any sentence the clients would
  ...
  every field is drift-tested by `browser/tests/llm/chat/systemPromptAsset.test.js`.
...
- **Generation — the prompt is generated but committed.** Each client assembles its prompt's ops
  ...
  (`browser/tests/llm/chat/systemPromptAsset.test.js`, "§13 regen") re-assembles that from the registry —
