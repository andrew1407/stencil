...
3. Normalizer + executor, per surface in the op's profiles (**the validator is table-driven —
   you write no schema code**): browser `js/llm/plan/parser.js` + `js/console/stencilApi.js`;
   desktop `src/llm/plan/opPlan.cpp` + `src/llm/plan/executor/planExecutor.cpp`; cli
   `src/llm/opplan.zig`; mcp `src/opplan/{parse,actions}.rs` + `src/opplan/lower/`; bot
   `Application/Llm/OpPlanParser*.cs` + `PromptService.Actions.cs`; pystencil
   `pystencil/llm/plan/{parse,execute}.py`; extension `src/llm/op/plan.js`.
...
5. If the command is part of the shared console profile, mirror it in
   `pystencil/pystencil/cli/commands/`, declared through `cli/registry.py`. Update
   `cli/README.md`.
...
1. `desktop/src/dialogs/<feature>/<name>Dialog.{hpp,cpp}` — chrome from
   `support/modal/modalChrome.hpp`, reveal from `support/modal/modalReveal.hpp`. Measure
   per-state heights in `showEvent`, not from a constructor `sizeHint` (a hidden widget's
   hint is stale).
2. Add the sources to `desktop/cmake/StencilSources.cmake`; a new folder joins
   `STENCIL_GUI_DIRS` there, or its includes carry the prefix.
3. Wire the action in `desktop/src/app/actions/MainWindowActions.cpp` and
   `MainWindowMenus.cpp`.
4. A shortcut goes in `browser/js/config/hotkeysConfig.json` — canonical — and reaches the
   desktop through the qrc alias.
5. Styling goes in the shared sheet in `support/theme/theme.cpp`, never `setStyleSheet` on the
   widget.
...
3. The mapping in each client, using its platform's built-in HTTP (**no new dependency**):
   ...
   `src/llm/client/LlmClient.cpp`; cli `src/llm/wire.zig` + `transport.zig`; mcp
   `src/llmtransport/`; bot `Infrastructure` ring, `Llm/HttpLlmClient.cs`; pystencil
   `pystencil/llm/client.py`; server `server/internal/llm/`.
4. Settings UI per surface (browser `js/llm/settings.js`, desktop
   `dialogs/settings/LlmSettingsForm.cpp`, extension `src/llm/settings.js`, cli `/llm`).
