...
|---|---|---|---|
| 1 | browser | `cd browser && npm test` | 2509 pass, 0 fail (~2 s) |
| 2 | extension | `cd extension && npm test` | 1215 pass, 0 fail (~2 s) |
...
| 4 | cli | `cd cli && zig build test --summary all` | 365 pass (~6 s) |
...
| 8 | mcp | `cd mcp && CARGO_BUILD_JOBS=2 cargo test --locked -j 2` | all suites 0 failed |
| 9 | desktop | see below | 45/45 |
...
nice -n 10 ctest --test-dir build -j1 -E mainwindow_gui --output-on-failure   # 44 targets, ~51 s
