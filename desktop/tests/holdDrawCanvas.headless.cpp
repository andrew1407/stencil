...
int main(int argc, char** argv) {
  ...
  // and starts a stroke, and with the RECT tool selected that must not be a LINE.
  {
    ...
    check(canvas.getCurrentLine().points.empty(), "holding with the rect tool starts no line");
    release();

    canvas.setDrawMode(CanvasWidget::DrawMode::LINE);
    hold();
    check(canvas.getIsDrawing() && canvas.getCurrentLine().points.size() == 1,
    ...
    canvas.setDrawMode(CanvasWidget::DrawMode::RECT);
    check(!canvas.getIsDrawing(), "starting from not-drawing");
    ...
    check(canvas.getIsDrawing(), "the rect press turned drawing on by itself");
    ...
    check(canvas.getLines().size() == 1, "…and the sweep really made a rect");
    check(canvas.getLines().size() == 1 && canvas.getLines()[0].points.size() == 4 &&
              canvas.getLines()[0].locked,
  ...
  // so both routes now go through tryCloseShapeAt.
  {
    ...
    t.start();
    while (t.elapsed() < 320) QCoreApplication::processEvents(QEventLoop::AllEvents, 10);
    check(canvas.getIsDrawing() && canvas.getCurrentLine().points.size() == 1, "the hold seeded a stroke");
    restAt(QPoint(200, 60));
    restAt(QPoint(120, 160));
    check(canvas.getCurrentLine().points.size() == 3, "three corners dropped");
    restAt(first);                       // …and back onto the first point
    check(!canvas.getIsDrawing(), "closing the shape ended the stroke");
    ...
    const bool committed = canvas.getLines().size() == 1;
    check(committed, "the closed shape was committed");
    check(committed && canvas.getLines()[0].locked,
          "…as a LOCKED area, not an open line that looks shut");
    check(committed && canvas.getLines()[0].points.size() == 4,
          "three corners plus the closing point");
    check(committed && (canvas.getLines()[0].fillColor == "transparent" ||
                        canvas.getLines()[0].fillColor.empty()),
    ...
    check(canvas.getSelectedLineIdx() == -1, "closing a shape selects nothing");
  ...
  // the first point a ~3px target at 25% zoom.
  {
    ...
    check(canvas.getCurrentLine().points.size() == 3, "three points down at 25% zoom");
    ...
    check(!canvas.getIsDrawing(), "the click closed the shape");
    check(canvas.getLines().size() == 1 && canvas.getLines()[0].locked,
