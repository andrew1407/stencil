// The selection-delete paths the Alt+Delete / Alt+Shift+Delete shortcuts invoke, the zoom-aware hit
// thresholds, and the point colour that resolves at draw time. All on the same live widget.
...
void deleteAndColourPaths(CanvasWidget& canvas) {
  ...
  canvas.setLines({a, b});
  check(canvas.getLines().size() == 2, "two lines seeded");

  std::printf("Alt+Shift+Delete path (deletePoint of the focused point):\n");
  canvas.selectLineAt(10, 10);
  check(canvas.getSelectedLineIdx() == 0, "line 0 selected by hit-test");
  check(canvas.getSelectedPoint() == 0, "point 0 focused");
  canvas.deletePoint(canvas.getSelectedPoint());
  check(canvas.getLines().size() == 2 && canvas.getLines()[0].points.size() == 2,
        "one point removed from line 0 (2 points remain)");

  std::printf("Alt+Delete path (deleteSelectedLine):\n");
  canvas.selectLineAt(100, 100);
  check(canvas.getSelectedLineIdx() == 1, "line 1 selected");
  canvas.deleteSelectedLine();
  check(canvas.getLines().size() == 1, "selected line removed (1 remains)");
  check(canvas.getSelectedLineIdx() == -1, "selection cleared after delete");
  ...
  std::printf("deletePoint erases the line it shows, not lines.back():\n");
  ...
  check(canvas.getSelectedLineIdx() == 0 && canvas.getSelectedPoint() == 0,
        "line 0 / point 0 selected");
  canvas.deletePoint(0);         // empties line 0 — it must be the one erased
  check(canvas.getLines().size() == 1, "one line remains");
  check(!canvas.getLines().empty() && canvas.getLines()[0].points.size() == 2,
  ...
  check(canvas.getCurrentLine().pointColor == "#ff0000",
  ...
  canvas.startDrawingMode();
  check(canvas.getCurrentLine().pointColor == "#00ff00", "explicit setting is kept");
  canvas.stopDrawingMode();
  ...
  check(canvas.getLines()[0].color == "#0000ff", "stroke recoloured");
  check(canvas.getLines()[0].pointColor == "#ff0000",
