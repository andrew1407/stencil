// Walks the shared persistence conformance corpora against the real desktop
// io/fileStore implementations:
//   • llm/fixtures/chatDoc   — §12.1 chat documents (buildChatDoc/parseChatDoc)
//   • fixtures/layout        — layout envelope (buildLayoutJson) + tolerant
//                              line parsing (linesFromJson vs expectFilled)
//   • fixtures/stencilProject — .stencil files (parseProjectFile)
// The corpora pin the BROWSER reference; measured desktop differences live in
// tests/fixtureOverrides.json (never in the shared fixtures).
//
// Not walkable on desktop (structural, not divergence):
//   • chatDoc savedAt tolerance — parseChatDoc returns messages only; savedAt
//     is supplied at build time, so the savedAt-coercion pins have no desktop
//     read path. The walker compares the messages half of expectParsed.
//   • layout expectKeyOrder — QJsonObject serializes alphabetically; desktop
//     never promises the browser's LAYOUT_FIELDS key order.
//   • layout expectSanitized / expectLinesSameRef — browser-internal shapes;
//     desktop parses straight to core::Line (the expectFilled contract).
