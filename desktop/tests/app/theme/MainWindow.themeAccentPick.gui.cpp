 ...
 private slots:
  ...
  void logoAccentPopoverPicksDirectly() {
    ...
    QTimer::singleShot(120, &win, [&] {
      ...
      stickySurvivedAlt = win.pop.active && !win.pop.active->isHidden();
      ...
      if (pop) {
        ...
        if (!row) { picksOk = false; }
          ...
          row->click();
          settle([&] { return win.settings.accentColor == key; }, 50);
          picksOk = win.settings.accentColor == key;                     // applied
          escapeClosedAfterPicks = !win.pop.active || win.pop.active->isHidden();  // closed
          lastPick = key;
        }
      }
    ...
    QTimer::singleShot(120, &win, [&] {
      ...
      releaseClosed = !win.pop.active || win.pop.active->isHidden();
      if (win.pop.active && !win.pop.active->isHidden()) win.pop.active->reject();
    });
    ...
    QTimer::singleShot(120, &win, [&] {
      if (win.pop.active) {
        ...
        promoted = win.pop.active && !win.pop.active->isHidden();
      }
      if (win.pop.active && !win.pop.active->isHidden()) win.pop.active->reject();
    });
    QTest::keyPress(&win, Qt::Key_Alt);
    ...
    QTimer::singleShot(120, &win, [&] {
      if (win.pop.active) {
        ...
        deactClosed = !win.pop.active || win.pop.active->isHidden();
      }
      if (win.pop.active && !win.pop.active->isHidden()) win.pop.active->reject();
      QTest::keyRelease(&win, Qt::Key_Alt);
    });
    ...
    QTest::keyPress(&win, Qt::Key_Alt);
    QTest::keyRelease(&win, Qt::Key_Alt);
    QVERIFY2(!win.pop.active, "Alt away from the icons must not open a popover");
