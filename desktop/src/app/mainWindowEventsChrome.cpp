// eventFilter chain, observers only: the pointer chrome (scrollbar auto-hide, the
// not-allowed cursor over a disabled control) and the dock chrome (the canvas↔panel grip and
// the chat dock's resize edge). Neither consumes — see mainWindowEvents.cpp for the order.
