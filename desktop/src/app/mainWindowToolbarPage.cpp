...
namespace stencil::gui {
  ...
  void MainWindow::buildPageFormulaToolbar() {
    ...
    // f(x,y) sits between PAGE and DATA as its own fenced section (the browser writes a .ctrl-
    // sep).
    // The x/y inputs belong to the pill's section: makeToolSection gives every control one height
    // and Qt::AlignVCenter, like the browser's flex row.
    buildFormulaFields();
    addWrappedSeparator(row);
