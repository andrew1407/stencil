...
namespace stencil::gui {
  ...
  void MainWindow::openChatCompact(QWidget* anchor) {
    ...
    if (chatCompactShowing() && chatDock->geometry() == compactChatRect(anchor)) {
      chatDock->raise();
    ...
    if (chatDock->isVisible()) {
      const int outMs = chatDock->isFloating() ? WINDOW_DISMISS_MS : CHAT_SLIDE_OUT_MS;
  ...
  void MainWindow::setChatCompactPopover(bool on) {
    ...
    if (chatDock) chatDock->setCompactPopover(on);
  }

  bool MainWindow::chatCompactShowing() const {
    return chatCompactPopover && chatDock && chatDock->isFloating() &&
           chatDock->isVisible();
  }
