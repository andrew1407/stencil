...
namespace stencil::gui {
  ...
  // Browser .modal-popover .app-modal: the compact shape is 420px wide whatever it holds,
  // and takes that width from sizeHint — so the FIELD has to ask for it, or the window
  // comes out a narrow column of whatever its text happens to need.
  ...
  inline void finishProjectMetaDialog(QDialog* dlg, ModalChrome& chrome,
    ...
    // A MINIMUM, never a fixed width (sizeModalTall's rule): a fixed one leaves the window
    // resizable only vertically, so its side and corner grips are dead — every other window
    // here takes a drag from any edge. The floor is the compact shape's own width.
    dlg->setMinimumWidth(POPOVER_W);
    // execMaybePopover reparents the dialog into its overlay, which drops the focus the
    // constructor set — so the field lost its accent ring in the compact shape. A 0-timer
    // lands once the loop is running, after any reparenting.
    if (focusTarget)
