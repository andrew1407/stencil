...
namespace stencil::gui {
  ...
  // capped by previewCapH_ once the picture has had to give up room (shrinkPreviewToFit).
  // From width(), the WINDOW's own: bodyContent_'s width is laid out FROM the stage this
  ...
  QSize OpenImageDialog::previewFitBox() const {
    ...
    if (previewCapH_ > 0) h = std::min(h, previewCapH_);
    return { w, h };
  }
  ...
  int OpenImageDialog::shrinkPreviewToFit(int over) {
    ...
    return std::max(wantedHeight(), floorH_);
  }

  // A wider window fits a bigger preview. floorH_ <= 0: showEvent's own first-show resize
  ...
  void OpenImageDialog::resizeEvent(QResizeEvent* event) {
    ...
    if (!bodyContent_ || !previewLabel_ || measuring_ || !measured_ || floorH_ <= 0) return;
    const int w = width();
    if (w == previewFitWidth_) return;
    previewFitWidth_ = w;
    previewCapH_ = 0;   // a new width, a fresh fit: the refit below re-derives any cap
    applyPreviewFit();
    ...
    if (!previewFitRefitPending_) {
      previewFitRefitPending_ = true;
      QPointer<OpenImageDialog> guard(this);
      QTimer::singleShot(0, this, [guard] {
        if (!guard) return;
        guard->previewFitRefitPending_ = false;
