...
| `vscode-extension/` | VS Code editor support for `.stc` and `.stcjs` (and a `.stencil` project icon) — spawns the `cli/` binary, hands scripts to `browser/` over `#stencil=` or drives its console through VS Code's built-in JS debugger, and carries a byte-equal copy of `browser/js/core/script*.js` for in-editor parsing | no |
