...
|---|---|---|---|
| **browser** | none to run (ES modules); optional `npm run build` → single-file `stencil.html` | `cd browser && npm test` | `npm run serve` (→ http://localhost:8080); single: `node --test tests/<file>.test.js` |
...
`.github/workflows/ci.yml` runs fourteen independent jobs on push/PR to `main` — each gated by a leading `changes` job on the paths it builds or reads (its own tree plus the cross-tree inputs it compiles, embeds or pins parity against; `*.md` never counts; an edit to `ci.yml`, a manual run or a push whose old tip is gone runs everything): browser (JS), **single-file** (`npm install` + `npm run build` in `browser/`, which self-verifies the emitted `stencil.html` needs no sibling file and carries no local file; no lockfile is tracked, so it installs with `npm install`), extension (JS), core (C++ + Doctest), desktop (Qt + headless tests, **including the `stencil_mainwindow_gui` QtTest e2e**), **wasm** (builds the core fresh with Emscripten and runs the parity test against it), cli (Zig), **pystencil** (Python stdlib tests; recompiles core via `build.py`), mcp (Rust, builds the CLI first for its gated e2e), **server** (Go build + `go test -race`, with Postgres + Redis service containers for the gated store/bus integration tests), **bot** (.NET build + the offline xUnit suite for the Telegram bot), **guard-hook** (the `.claude/hooks` PreToolUse guard's own `node --test` suite), **e2e** (Node/Playwright — brings up db+redis+server via docker compose and drives the real browser app + unpacked extension + server binary over their wire protocols; see `e2e/README.md`), and **docker-images** (a matrix that `docker build`s all five shipped Dockerfiles — browser/cli/mcp/bot from the repo root, server from `./server` — so a broken image never ships; behavior is covered elsewhere, this guards packaging). The wasm job is what catches core/JS-fallback divergence. `desktop-packages.yml` builds desktop packages for macOS/Windows/Linux on `v*` tags (always) and on pushes/PRs that touch the app's inputs (`desktop/`, `core/`, the qrc-embedded `browser/js/config/` + `favicon.svg`). `pages.yml` deploys the browser app to GitHub Pages on push to `main` (and on demand): it builds the wasm core fresh and serves `browser/` as the site root, with the single-file `stencil.html` alongside — inert until Pages is enabled for the repo with source "GitHub Actions".
