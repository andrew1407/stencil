...
// STENCIL_DOCS_CAPTURE=ON; run through usecases/capture/desktop.mjs, which reads
...
int main(int argc, char** argv) {
  ...
  if (outDir().isEmpty() || framesDir().isEmpty()) {
    std::fprintf(stderr, "set STENCIL_DOCS_OUT and STENCIL_DOCS_FRAMES (usecases/capture/desktop.mjs does)\n");
    return 2;
  }
  QDir().mkpath(outDir());
