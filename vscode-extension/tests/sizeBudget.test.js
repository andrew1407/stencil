// The size + comment ratchet for the VS Code extension: no new oversized module, no listed file
...
// Scope: .js under vscode-extension/src and vscode-extension/tests. Paths are repo-relative so every
...
const SCOPE = ['vscode-extension/src', 'vscode-extension/tests'];
...
test('test-count floor: the suite still discovers and runs its whole tree',
  { skip: process.env[INNER_RUN] ? 'inner count run' : false }, () => {
    ...
    assert.ok(count >= TEST_FLOOR,
      `vscode-extension suite collapsed to ${count} tests, floor is ${TEST_FLOOR}`);
  });
