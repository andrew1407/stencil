...
// lock that makes it the only thing the page listens to until Escape or a click ends it.
// Browser twin: js/ui/logo/stage.js — the same body but for the page's own modals and mark.
...
// Only from the bare window: no stage already up and no dialog of the page's own on top.
const MODALS = '.confirm-overlay:not([hidden]), .dialog-back';
export const logoStageAllowed = (doc = globalThis.document) => {
  ...
  return !doc.querySelector?.(MODALS);
};
...
export const markOrigin = (doc = globalThis.document) => {
  const r = doc?.querySelector?.('.logo-wrap')?.getBoundingClientRect?.();
