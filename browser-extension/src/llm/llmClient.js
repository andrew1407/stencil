...
// Extension copy of browser/js/llm/llmClient.js — the extension can't import from
// browser/ (separate subproject), so the client is duplicated here. One
// chat({ system, messages }) method over the three providers; messages use the
// stencil-server DTO shape as the canonical form ({ role, text, images:
// [{ mediaType, data }] }). fetch is injected for `node --test`.
//
// SHARED with browser/js/llm/llmClient.js: the two copies must stay identical
// below this header (tests/portParity.test.js pins them); anything per-surface
// lives in llmSurface.js.
