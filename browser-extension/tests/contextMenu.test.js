...
test('MENU_ITEMS: a "Stencil" parent per group holds every item; Preview nests one deeper', () => {
  ...
  // extension name: the STATIC native one, and the DYNAMIC probe-revealed background one.
  ...
  // Native items are scoped to native contexts; none is on 'page'/'all' (only the root carries
  // 'all'), so they never appear on plain elements.
  ...
  // The always-on native items carry no `visible` flag; the dynamically-gated groups — background,
  // Preview, and the desktop hand-off gated on a configured scheme — do.
...
// Chrome decides a PARENT's visibility from its own `contexts` alone, never from whether a child
// ended up visible, so a contexts:['all'] root paints as "Stencil ▸ (empty)".
