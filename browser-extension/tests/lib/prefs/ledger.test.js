...
test('trackableSource: only http(s) URLs are tracked', () => {
  assert.equal(trackableSource('https://a.example/i.png'), true);
  assert.equal(trackableSource('http://a.example/i.png'), true);
  assert.equal(trackableSource('data:image/png;base64,AA'), false);
  assert.equal(trackableSource('blob:https://a.example/x'), false);
...
test('matchEntries: prefers exact source URL', () => {
  const entries = [
    e({ source: 'https://a.example/i.png', name: 'i.png', resource: 'https://a.example/p1' }),
    e({ source: 'https://a.example/i.png', name: 'i.png', resource: 'https://a.example/p2' }),
    e({ source: 'https://b.example/j.png', name: 'j.png' }),
  ];
  const m = matchEntries(entries, 'https://a.example/i.png', 'whatever.png');
  assert.equal(m.length, 2);
  assert.ok(m.every(x => x.source === 'https://a.example/i.png'));
});

test('matchEntries: falls back to name only when no source given', () => {
  const entries = [
    e({ source: '', name: 'photo.png' }),
    e({ source: 'https://a.example/photo.png', name: 'photo.png' }),
  ];
...
test('matchEntries: tolerates non-array input', () => {
  assert.deepEqual(matchEntries(null, 'https://a.example/i.png', 'i'), []);
...
test('originOf: extracts origin, empty on garbage', () => {
  ...
  assert.equal(originOf('https://a.example/i.png?q=1'), 'https://a.example');
...
test('reconcileLedger: drops entries whose project was deleted (same editor)', () => {
  const entries = [led('https://a.example/keep.png'), led('https://a.example/gone.png')];
  const projects = [{ source: 'https://a.example/keep.png' }];
  const out = reconcileLedger(entries, projects, 'http://localhost:8080', NOW);
  assert.equal(out.length, 1);
  assert.equal(out[0].source, 'https://a.example/keep.png');
});

test('reconcileLedger: leaves entries for a DIFFERENT editor untouched', () => {
  const entries = [led('https://a.example/x.png', 'https://editor.example.com/')];
...
test('reconcileLedger: keeps fresh entries (editor may not have saved yet)', () => {
  const fresh = led('https://a.example/just-opened.png', 'http://localhost:8080/', NOW); // t == now
...
test('reconcileLedger: re-derives count from the number of live projects for the source', () => {
  const entries = [led('https://a.example/x.png')]; // count 1
  // Two live projects (an original + an "add a copy") share this source.
  const projects = [{ source: 'https://a.example/x.png' }, { source: 'https://a.example/x.png' }];
  const out = reconcileLedger(entries, projects, 'http://localhost:8080', NOW);
  assert.equal(out.length, 1);
...
test('reconcileLedger: count moves back down when copies are removed', () => {
  const entries = [e({ source: 'https://a.example/x.png', name: 'i.png', editorUrl: 'http://localhost:8080/', t: 0, count: 5 })];
  const out = reconcileLedger(entries, [{ source: 'https://a.example/x.png' }], 'http://localhost:8080', NOW);
...
test('reconcileLedger: empty registry prunes all stale same-editor entries', () => {
  const entries = [led('https://a.example/a.png'), led('https://a.example/b.png')];
  const out = reconcileLedger(entries, [], 'http://localhost:8080', NOW);
...
test('reconcileLedger: tolerates bad input and never reorders survivors', () => {
  ...
  const entries = [led('https://a.example/1.png'), led('https://a.example/2.png')];
  const projects = [{ source: 'https://a.example/2.png' }, { source: 'https://a.example/1.png' }];
  const out = reconcileLedger(entries, projects, 'http://localhost:8080', NOW);
  assert.deepEqual(out.map(x => x.source), ['https://a.example/1.png', 'https://a.example/2.png']);
});
