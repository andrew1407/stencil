...
import { resolveHighlightColor, highlightColorValue, ACCENT_HEX, DEFAULT_HL } from '../src/lib/highlight/color.js';
...
test('highlightColorValue: uses the passed settings; no page accent → default', async () => {
  assert.equal(await highlightColorValue({ highlightColor: '#ff0000' }), '#ff0000');
  assert.equal(await highlightColorValue({ highlightColor: 'theme' }), DEFAULT_HL);   // no window.StencilAccent in Node
});
