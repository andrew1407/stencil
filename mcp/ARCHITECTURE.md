...
| `OpPlan` | the validated model reply: `reply`, `actions`, `variants`, `warnings`, `chat_only`, `ask`; canonical shape is `browser/js/llm/plan/plan.js` over `opRegistry.json` | per round, from `parse_op_plan` | lowered to `EditRequest`s; `AskCard` reported as data |
