...
// ── Restoring a document another surface wrote ──────────────────────────────
// The §12 doc rides the .stencil file and the server's `chat` file kind, so a doc
// built by a surface that serialised its MODEL history (the desktop's bug) can land
// in this editor. It must restore as a conversation — no continuation note, no raw
// plan — in the transcript AND in the replay history seeded from it.
