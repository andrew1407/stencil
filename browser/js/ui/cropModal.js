...
export class StencilCropModal extends StencilElement {
    ...
    return `
        <div class="app-modal" style="width:auto;max-width:calc(100vw - 32px);">
            <div class="settings-header">
                <h2>✂ Crop Image</h2>
                <button class="app-modal-close" id="cropClose">✕ Close</button>
            </div>
            <div class="settings-body" style="display:flex;flex-direction:column;align-items:center;gap:12px;">
                <div id="cropStage" style="position:relative;display:inline-block;line-height:0;max-width:100%;background:#222;overflow:hidden;">
                    <img id="cropImageEl" alt="Crop preview" style="display:block;width:auto;height:auto;max-width:calc(96vw - 60px);max-height:calc(82vh - 180px);user-select:none;-webkit-user-drag:none;">
                    <div id="cropBox" style="position:absolute;box-sizing:border-box;border:2px solid #4da3ff;box-shadow:0 0 0 9999px rgba(0,0,0,0.45);cursor:move;display:none;">
                ...
                <div id="cropDims" style="font-size:13px;color:var(--text-muted);"></div>
            </div>
            <div class="settings-footer">
                <button id="cropOrientation" title="Swap album / portrait — flips the crop orientation">⤢ Album</button>
  ...
  static template() { return hostTag('stencil-crop-modal', 'id="cropModalOverlay" class="app-modal-overlay"', StencilCropModal.inner()); }
  ...
  wire(app) {
    ...
    const orientBtn = document.getElementById('cropOrientation');
    ...
    const { open, close } = wireModalShell(overlay, null, document.getElementById('cropClose'), {
    ...
    document.getElementById('cropImage').addEventListener('click', openCrop);
