...
// The app's one controller (js/llm/session.js), created lazily so it consumes the
...
// Provider status: the dot on the "…" trigger plus its tooltip (ui/statusTip.js).
...
// An image dragged from another page carries no File, just a URL in uri-list/html.
    ...
    let gestures = null;
// ui/dock.js; playDust and the pill rects are declared below, so they cross as thunks.
...
// display:none cannot animate. One clock for every dock.
    const CLOSE_MS = 510;
// Fullscreen shows a clone of the toolbar (ui/layer.js) with the same id, so
